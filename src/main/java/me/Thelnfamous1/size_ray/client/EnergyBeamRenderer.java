...
import me.Thelnfamous1.size_ray.EnergyBeam;
...
public class EnergyBeamRenderer<T extends EnergyBeam> extends EntityRenderer<T> {
    ...
    public void render(T energyBeam, float pEntityYaw, float pPartialTicks, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight) {
        ...
        if(owner != null){
            Either<BlockHitResult, EntityHitResult> hitResult = SizeRayUtil.getBeamHitResult(owner, SizeRayUtil.MAX_HIT_DISTANCE);
            if(hitResult.left().isPresent() && hitResult.left().get().getType() == HitResult.Type.MISS){
                return;
            }
            double distanceTo = hitResult.map(bhr -> bhr.distanceTo(owner), ehr -> ehr.distanceTo(owner));
            drawBeams(energyBeam, owner, pPoseStack, distanceTo, SPEED_MODIFIER, pPartialTicks);
        }
    }

    private static void drawBeams(EnergyBeam energyBeam, Entity owner, PoseStack pPoseStack, double distance, float speedModifier, float partialTicks) {
        ...
        int color = energyBeam.getColor();
        float innerR = ((float)(color >> 16 & 255) / 255.0F);
        float innerG = ((float)(color >> 8 & 255) / 255.0F);
        float innerB = ((float)(color & 255) / 255.0F);
